...
export default function VerificationForm({ formData, handleChange, errors }) {
  const renderFileUpload = (fieldName, label, description) => {
    ...
    return (
      <div className="border rounded-lg p-3 sm:p-5 bg-white shadow-sm hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start">
          ...
          {file && (
            <div className="flex items-center text-green-500">
              <CheckCircle size={16} className="mr-1 sm:size-18" />
              <span className="text-xs sm:text-sm font-medium">Uploaded</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-3 sm:mt-4">
          {file ? (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-50 p-2 sm:p-3 rounded-md">
              <span className="text-xs sm:text-sm truncate max-w-[150px] sm:max-w-[200px] font-medium mb-2 sm:mb-0">
                {file.name}
              </span>
              <div className="flex space-x-2">
                <button
                  type="button"
                  className="px-2 py-1 sm:px-3 sm:py-1.5 text-xs bg-blue-50 text-blue-600 rounded-md hover:bg-blue-100 transition-colors"
                ...
                <button
                  type="button"
                  className="px-2 py-1 sm:px-3 sm:py-1.5 text-xs bg-red-50 text-red-600 rounded-md hover:bg-red-100 transition-colors"
                  onClick={() => {
                    const mockEvent = {
                      target: {
                        name: fieldName,
            ...
            <div className="mt-2">
              ...
              <label
                htmlFor={fieldName}
                className="inline-block px-3 py-2 sm:px-4 sm:py-2.5 text-xs sm:text-sm bg-blue-500 text-white rounded-md cursor-pointer hover:bg-blue-600 transition-colors shadow-sm"
              >
                Upload Document
              </label>
            </div>
          )}
          {errors[fieldName] && <p className="text-xs sm:text-sm text-red-500 mt-1">{errors[fieldName]}</p>}
        </div>
      </div>
    )
  }
